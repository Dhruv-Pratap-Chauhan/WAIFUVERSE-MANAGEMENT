...
from .. import pbot as Dhruv,OWNER_ID
...
@Dhruv.on_message(filters.command("gban") & filters.user(OWNER_ID) & ( filters.group | filters.channel) )
...
@Dhruv.on_message(filters.command("ungban") & filters.user(OWNER_ID) & ( filters.group | filters.channel) )
async def  unbanuser(b,message):
    ...
    try:
        ...
        await message.reply_text(f"failed due to {e}")
@Dhruv.on_message(filters.command("gbanlist") & filters.user(OWNER_ID))
