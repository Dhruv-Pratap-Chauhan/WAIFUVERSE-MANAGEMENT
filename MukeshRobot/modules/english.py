...
async def _(event):
    if len(event.text.split()) < 2:
        return await event.reply("ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴡᴏʀᴅ ᴛᴏ ᴅᴇғɪɴᴇ!")
    text = event.text[len("/define ") :]
    word = f"{text}"
    let = dictionary.meaning(word)
    if not let:
        return await event.reply("ɴᴏ ᴍᴇᴀɴɪɴɢ ғᴏᴜɴᴅ ғᴏʀ ᴛʜɪs ᴡᴏʀᴅ.")
    set = str(let)
    jet = set.replace("{", "")
    net = jet.replace("}", "")
    got = net.replace("'", "")
    if not got:
        got = "ɴᴏ ᴍᴇᴀɴɪɴɢ ғᴏᴜɴᴅ."
...
async def _(event):
    if len(event.text.split()) < 2:
        return await event.reply("ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴡᴏʀᴅ!")
    text = event.text[len("/synonyms ") :]
    word = f"{text}"
    let = dictionary.synonym(word)
    if not let:
        return await event.reply("ɴᴏ sʏɴᴏɴʏᴍs ғᴏᴜɴᴅ.")
    set = str(let)
    jet = set.replace("{", "")
    net = jet.replace("}", "")
    got = net.replace("'", "")
    if not got:
        got = "ɴᴏ sʏɴᴏɴʏᴍs ғᴏᴜɴᴅ."
    await event.reply(got)
...
async def _(event):
    ...
    if not let:
        return await event.reply("ɴᴏ ᴀɴᴛᴏɴʏᴍs ғᴏᴜɴᴅ.")
    set = str(let)
    jet = set.replace("{", "")
    net = jet.replace("}", "")
    got = net.replace("'", "")
    if not got:
        got = "ɴᴏ ᴀɴᴛᴏɴʏᴍs ғᴏᴜɴᴅ."
    await event.reply(got)
