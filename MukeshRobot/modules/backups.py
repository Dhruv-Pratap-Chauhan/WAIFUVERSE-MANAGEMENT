...
def export_data(update, context):
    ...
    with open("DhruvRobot{}backup.json".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("DhruvRobot{}backup.json".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `DhruvRobot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("DhruvRobot{}backup.json".format(chat_id))  # Cleaning file
...
__mod_name__ = "Bᴀᴄᴋᴜᴘ"
# Removed __help__ to hide from public management list
