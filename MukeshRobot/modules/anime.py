...
def anime_quote():
    ...
    try:
        ...
        if response.status_code == 200:
            dic = response.json()
            if "data" in dic: # Some versions of the API return data in a 'data' key
                dic = dic["data"]
            quote = dic.get("quote", "No quote found.")
            character = dic.get("character", "Unknown")
            anime = dic.get("anime", "Unknown")
